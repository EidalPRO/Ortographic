...
function cargarReactivo() {
    ...
    if (preguntasMostradas === reactivos.length) {
        ...
        do {
            // console.log(reactivos.length);
            id = Math.floor(Math.random() * reactivos.length);
        } while (numerosGenerados.includes(id));
        numerosGenerados.push(id);
...
function avanzarBarraProgreso() {
    var porcentaje = (preguntasRespondidas / reactivos.length) * 100;
